...
const switchVideo = (index) => {
  ...
  document.getElementById('video-iframe').src = 'https://www.youtube-nocookie.com/embed/' + video.videoId + "?controls=0&origin='cumathsoc.ca'";
};

const getListFmt = (list) => {
  return list.map((item, index, list) => {
    return "<p onclick=switchVideo('" + index + "')>" + item.title + '</p>'; //onclick and pass videoid
  }).join('');
};
